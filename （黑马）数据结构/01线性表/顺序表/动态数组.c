...
Status InsertElem(SqList *L, int i, Elemtype e)
{
    ...
    {
		newspace = (Elemtype*) realloc(L->elem, (L->size + INCREMENT_SIZE) * sizeof(Elemtype));//在原来内存空间上 新 开闭一段存储空间 
        if (!newspace)//开辟出错  可能到了 系统空间 
        {
            return ERROR;
        }
        L->elem = newspace;//新的存储空间  元素 
